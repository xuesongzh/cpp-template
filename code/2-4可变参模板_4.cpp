...
//一个特殊的特化版本，看起来象全特化，不是全特化，可变参模板不存在全特化
template <>
class myclasst<> {
 public:
    myclasst() {
...
void myfunctuple(const tuple<T...>& t) {
    myclasst4<0, sizeof...(T), T...>::mysfunc(t);  //这里注意，第一个参数是0，表示计数从0开始
}
}  // namespace _nmsp2
...
int main() {
    ...
    //(2)如何用tuple和递归调用展开参数包:
    // 实现思路，有一个计数器（变量），从0开始，每处理一个参数，计数器+1，一直到把所有参数处理完。
    // 最后提供一个模板偏特化，作为递归调用结束。
